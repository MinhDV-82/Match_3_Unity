...
public static class CommonUtils
{
    ...
    public static BaseCandy.CandyColor GetCandyByColor(BaseCandy dot) {
        BaseCandy.CandyColor[] dotColors = (BaseCandy.CandyColor[])Enum.GetValues(typeof( BaseCandy.CandyColor));

        for (int i = 1; i < dotColors.Length; i++) {
            if (dotColors[i].Equals(dot.GetCurrentCandyColor())) return dotColors[i];
        }
